...
class BatchGenerator(object):
    def __init__(self, batch_size, seq_len):
        ...
        # Labels are padded to be of same length
        ...
        self.indices = np.random.choice(
            len(self.dataset), size=(batch_size,), replace=False)
        self.batches = np.zeros((batch_size,), dtype=np.int32)

    def next_batch(self):
        coords = np.zeros(
            (self.batch_size, self.seq_len + 1, 3), dtype=np.float32)
        sequence = np.zeros((self.batch_size, self.max_len,
                             self.num_letters), dtype=np.float32)
        ...
        for i in range(self.batch_size):
            if self.batches[i] + self.seq_len + 1 > self.dataset[self.indices[i]].shape[0]:
                ...
                reset_states[i] = 0.
                needed = True
            coords[i, :, :] = self.dataset[self.indices[i]
                                           ][self.batches[i]: self.batches[i] + self.seq_len + 1]
    ...
    @staticmethod
    def load_dataset():
        dataset = np.load(os.path.join('HandwrittenModel/data',
        ...
        for d in dataset:
            ...
            offs = d[1:, :2] - d[:-1, :2]
            ends = d[1:, 2]
            temp += [np.concatenate([[[0., 0., 1.]],
                                     np.concatenate([offs, ends[:, None]], axis=1)], axis=0)]

        dataset = temp
        labels = np.load(os.path.join('HandwrittenModel/data',
