...
from HandwrittenModel.utils import next_experiment_path
from HandwrittenModel.utility_batch_generator import BatchGenerator


# Seq_Length -- No. of timesteps
# Batch_Size -- No. of examples per training example
...
parser.add_argument('--window_mixtures',
                    dest='window_mixtures', default=10, type=int)
parser.add_argument('--output_mixtures',
                    dest='output_mixtures', default=20, type=int)
...
# Creates Synthesis Network


class WindowLayer(object):
    def __init__(self, num_mixtures, sequence, num_letters):
        # one-hot encoded sequence of characters -- [batch_size, length, num_letters]
        self.sequence = sequence
        self.seq_len = tf.shape(input=sequence)[1]
    ...
    def __call__(self, inputs, k, reuse=None):
        with tf.compat.v1.variable_scope('window', reuse=reuse):
            alpha = tf.compat.v1.layers.dense(inputs, self.num_mixtures, activation=tf.exp,
                                              kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.075), name='alpha')
            beta = tf.compat.v1.layers.dense(inputs, self.num_mixtures, activation=tf.exp,
                                             kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.075), name='beta')
            kappa = tf.compat.v1.layers.dense(inputs, self.num_mixtures, activation=tf.exp,
                                              kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.075), name='kappa')
            # a - Scale Parameter
            # b - Scale Parameter
            ...
            phi = tf.exp(-tf.square(self.u_range + k) * b) * \
                a  # [batch_size, mixtures, length]
            # [batch_size, 1, length]
            phi = tf.reduce_sum(input_tensor=phi, axis=1, keepdims=True)

            # whether or not network finished generating sequence
            finish = tf.cast(
                phi[:, 0, -1] > tf.reduce_max(input_tensor=phi[:, 0, :-1], axis=1), tf.float32)
...
class MixtureLayer(object):
    ...
    def __call__(self, inputs, bias=0., reuse=None):
        with tf.compat.v1.variable_scope('mixture_output', reuse=reuse):
            e = tf.compat.v1.layers.dense(inputs, 1,
                                          kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.075), name='e')
            pi = tf.compat.v1.layers.dense(inputs, self.num_mixtures,
                                           kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.075), name='pi')
            mu1 = tf.compat.v1.layers.dense(inputs, self.num_mixtures,
                                            kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.075), name='mu1')
            mu2 = tf.compat.v1.layers.dense(inputs, self.num_mixtures,
                                            kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.075), name='mu2')
            std1 = tf.compat.v1.layers.dense(inputs, self.num_mixtures,
                                             kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.075), name='std1')
            std2 = tf.compat.v1.layers.dense(inputs, self.num_mixtures,
                                             kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.075), name='std2')
            rho = tf.compat.v1.layers.dense(inputs, self.num_mixtures,
                                            kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.075), name='rho')
...
class RNNModel(tf.compat.v1.nn.rnn_cell.RNNCell):
    ...
    def call(self, inputs, state, **kwargs):
        ...
        for layer in range(self.layers):
            ...
            with tf.compat.v1.variable_scope('lstm_{}'.format(layer)):
                output, s = self.lstms[layer](x, tf.compat.v1.nn.rnn_cell.LSTMStateTuple(state[2 * layer],
                                                                                         state[2 * layer + 1]))
                prev_output = [output]
            output_state += [*s]
...
def create_graph(num_letters, batch_size,
    ...
    with graph.as_default():
        coordinates = tf.compat.v1.placeholder(
            tf.float32, shape=[None, None, 3])
        sequence = tf.compat.v1.placeholder(
            tf.float32, shape=[None, None, num_letters])
        reset = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
        bias = tf.compat.v1.placeholder_with_default(
        ...
        def create_model(generate=None):
            ...
            with tf.compat.v1.variable_scope('model', reuse=generate):
                window = WindowLayer(
                    num_mixtures=window_mixtures, sequence=sequence, num_letters=num_letters)
                ...
                outs, states = tf.compat.v1.nn.dynamic_rnn(rnn_model, in_coords,
                ...
                with tf.control_dependencies([sp.assign(sc) for sp, sc in zip(rnn_model.states, states)]):
                    with tf.compat.v1.name_scope('prediction'):
                        ...
                        e, pi, mu1, mu2, std1, std2, rho = output_layer(
                            outs, bias)

                    with tf.compat.v1.name_scope('loss'):
                        coords = tf.reshape(out_coords, [-1, 3])
                        xs, ys, es = tf.unstack(
                            tf.expand_dims(coords, axis=2), axis=1)

                        mrho = 1 - tf.square(rho)
                        xms = (xs - mu1) / std1
                        yms = (ys - mu2) / std2
                        z = tf.square(xms) + tf.square(yms) - \
                            2. * rho * xms * yms
                        n = 1. / (2. * np.pi * std1 * std2 *
                                  tf.sqrt(mrho)) * tf.exp(-z / (2. * mrho))
                        ep = es * e + (1. - es) * (1. - e)
                        rp = tf.reduce_sum(input_tensor=pi * n, axis=1)

                        loss = tf.reduce_mean(
                            input_tensor=-tf.math.log(rp + epsilon) - tf.math.log(ep + epsilon))
                    ...
                    if generate:
                        ...
                        for param in [('coordinates', coordinates),
                            ...
                            tf.compat.v1.add_to_collection(*param)

                with tf.compat.v1.name_scope('training'):
                    steps = tf.Variable(0.)
                    learning_rate = tf.compat.v1.train.exponential_decay(0.001, steps, staircase=True,
                    ...
                    train_step = optimizer.apply_gradients(
                        zip(grad, var), global_step=steps)
...
def main():
    ...
    with tf.compat.v1.Session(graph=g) as sess:
        model_saver = tf.compat.v1.train.Saver(max_to_keep=2)
        if restore_model:
        ...
        summary_writer = tf.compat.v1.summary.FileWriter(
            experiment_path, graph=g, flush_secs=10)
        summary_writer.add_session_log(tf.compat.v1.SessionLog(status=tf.compat.v1.SessionLog.START),
        ...
        for e in range(epoch, num_epoch):
            ...
            for b in range(1, batches_per_epoch + 1):
                ...
                summary_writer.add_summary(
                    s, global_step=e * batches_per_epoch + b)
                print('\r[{:5d}/{:5d}] loss = {}'.format(b,
                                                         batches_per_epoch, l), end='')
